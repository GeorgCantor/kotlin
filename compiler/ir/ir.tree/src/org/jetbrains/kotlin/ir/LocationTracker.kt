...
object LocationTracker {
    ...
    val parameterAddedWhileThereAreExistingCalls = ConcurrentHashMap.newKeySet<List<StackTraceElement>>()
    val parameterRemovedWhileThereAreExistingCalls = ConcurrentHashMap.newKeySet<List<StackTraceElement>>()
    val argumentAddedForNonExistingParameter = ConcurrentHashMap.newKeySet<List<StackTraceElement>>()
    val argumentAddedForANewParameter = ConcurrentHashMap.newKeySet<List<StackTraceElement>>()

    private fun registerHook() {
        Runtime.getRuntime().addShutdownHook(Thread {
            listOf(
                ::parameterAddedWhileThereAreExistingCalls,
                ::parameterRemovedWhileThereAreExistingCalls,
                ::argumentAddedForNonExistingParameter,
                ::argumentAddedForANewParameter
            ).forEach { prop ->
                val name = prop.name
                val set = prop.get()
                println()
